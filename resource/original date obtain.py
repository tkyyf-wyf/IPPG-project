...
if __name__ == '__main__':
    # 如果模块本身被运行则执行下面代码
    FRAME_MIN = 0  # 设置截取秒数
    FRAME_MAX = 6000  # 设置截取最大秒数
    path = 'D:/LUT/dateset/UBFC-PHYS/s1/vid_s1_T1.avi'  # 导入视频路径（注意不要出现\u）
    roi_list = data_obtain(path, FRAME_MIN, FRAME_MAX)  # 测试函数是否正常
    with open('D:\\LUT\\IPPG-project-clone\\data\\vid_s1_T1_data.csv', 'x') as file:
